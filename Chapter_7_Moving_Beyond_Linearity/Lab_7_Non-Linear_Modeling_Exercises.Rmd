...
```{r}
a <- Y - βˆ1 * X1
coefficient_list <- lm(a ~ ., data = df %>% select(everything(), -X1))$coeff
coefficient_list
```
...
coefficient_list <- lm(a ~ X1 + X3)$coeff
coefficient_list
```
...
coefficient_list <- lm(a ~ X1 + X2)$coeff
coefficient_list
```
...
coefficient_list <- lm(a ~ X2 + X3)$coeff
coefficient_list
```
...
predictors <- matrix(data = NA, nrow = 100, ncol = 100)

for (i in  seq(1:100)) {
  for (j in seq(1:100)) {
    predictors[i, j] <- rnorm(1)
  }
}
```
...
# predictors[,]
# true_coefficients
Y <- intercept + true_coefficients * predictors + ε
```
...
```{r}
βˆ1 <- 1
collected_intercept <- βˆ1
# collected_estimted_coefficients[1] <- 
a <- Y - βˆ1 * predictors[,1]
estimated_coefficients <- lm(a ~ predictors[, -1])$coeff
collected_estimated_coefficients[2] <- estimated_coefficients[2] # ???

for(i in seq(2, 100)){
  a <- Y - estimated_coefficients[i] * predictors[,i]
  estimated_coefficients <- lm(a ~ predictors[, -i])$coeff
...
```{r}
near((true_coefficients - collected_estimated_coefficients), 0)
near(intercept - collected_intercept, 0)
mean((true_coefficients - collected_estimated_coefficients)^2)
```
```{r}
length(collected_estimated_coefficients)
```

```{r}
(true_coefficients - collected_estimated_coefficients)
```
