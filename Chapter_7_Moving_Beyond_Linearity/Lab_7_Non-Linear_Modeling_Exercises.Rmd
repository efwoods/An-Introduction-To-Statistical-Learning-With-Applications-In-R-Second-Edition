...
```{r}
# Remove Widowed
wage_not_widowed <- Wage %>% filter(maritl != "Widowed")
# wage_not_widowed
```
...
```{r}
gam.maritl <- gam(wage ~ maritl + jobclass + s(year, 5), data = Wage)
par(mfrow = c(1, 1))
...
```{r}
f_print(sprintf("There is a strong relationship between marital status & Information job class with respect to wage. There is a positive trend with increasing years with respect to wage."))
```
