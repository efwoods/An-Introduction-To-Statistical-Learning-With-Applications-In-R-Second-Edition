...
```{r results='hide'}
mean((true_intercept - βˆ0_c[i])^2) 
mean((true_coefficients[1] - βˆ1_c[i])^2)
mean((true_coefficients[2] - βˆ2_c[i])^2)
```
...
mean((true_coefficients[1] - βˆ1_c[i])^2)
mean((true_coefficients[2] - βˆ2_c[i])^2)
mean((true_coefficients[3] - βˆ3_c[i])^2)
```

```{r}
true_intercept - βˆ0_c[1]
true_coefficients[1] - βˆ1_c[1]
true_coefficients[2] - βˆ2_c[1]
true_coefficients[3] - βˆ3_c[1]
```
...
```{r}
βˆ1 <- 5
# Initialization & First Pass of Collecting Estimated Coefficients
a <- Y - βˆ1 * predictors[1]
...
```
```{r}
collected_estimated_coefficients
```
```{r}
i <- 1
```
