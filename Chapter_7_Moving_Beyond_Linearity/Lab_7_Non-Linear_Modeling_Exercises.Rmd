...
```{r}
# Input: Dataframe, x, y, title of the plot, title of the x-axis, title of the y-axis, metric, rss
# Note: Metric == "rss" will plot rss on the graph when rss is provided to the function.
...
create_custom_geom_point <- function(df, x, y, title, x_title, y_title, metric=NULL, rss = NULL, dof = NULL, pred = NULL){ #???
    if(metric == "rss" & !is.null(rss) & !is.null(dof)) {
      metric_label <- sprintf("rss: %0.3f\ndof: %0.0f", rss, dof) # rss
    } else {
      metric_label <- sprintf("r: %0.3f", cor(x,y)) # correlation
    }
    label <- df %>% summarise(x = max(x), y = max(y), label = metric_label)
    ggplot(df, aes(x, y)) +
      geom_point(color = custom_darkblue) + 
      theme_linedraw() +
      geom_point(se = FALSE, color = custom_darkblue) + 
      geom_line(aes(x, pred), color = custom_lightblue) + #????
      labs(title = title, x = x_title, y = y_title) + 
...
ggplot() + 
  ...
  labs(title = "NOx Concentration vs. Mean Distance to Employment Centers", x = "Weighted Mean of Distance to 5 Boston Employment Centers.", y = "Nitrogen Oxides Concentration (Parts Per 10 Million)")
```
...
for(i in 1:10) {
  ...
  print(create_custom_geom_point(boston, dis, nox, "Polynomial Fit of NOx vs. Distance to Employment Centers", "Weighted Mean of Distance to 5 Boston Employment Centers", "NOx Concentration (Parts Per 10 Million)", metric = "rss", rss = rss[i], dof = i, pred = lm.pred)) #???
}
# ???
# create_custom_geom_point <- function(df, x, y, title, x_title, y_title, metric=NULL, rss = NULL){


rss
# ggplot() + 
  # geom_point(aes(dis, lm.fit))
```
