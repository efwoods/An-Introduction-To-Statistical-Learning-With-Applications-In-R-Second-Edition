...
* **Question 10-b**: Make pairwise scatterplots of the predictors (columns) in this data set. Describe your findings. 
  * **Answer**: Per capita crime rate tends to increase with an increase in the percent of the lower status of the population and with an increase in higher proportions of homes built before 1940. Per capita crime rate also tends to increase with decreases in the median value of owner-occupied homes, the weighted mean distance to employment centers, and the average number of rooms per dwelling. Furthermore, there are impulses of increased crime rates with pupil-teacher ratios of approximately 20%, Full-value property tax rates per $10,000 of approximately 675, with a proportion of non-retail business acres per town of approximately 18%, and nitrogen oxide concentrations of approximately 0.7.
...
* **Question 10-c**: Are any of the predictors associated with per capita crime rate? If so, explain the relationship.
  * **Answer**: Per capita crime rate appears to associate with the median value of owner-occupied homes, the percent of lower status of the population, the weighted mean distance to employment centers, the proportion of owner-occupied units built before 1940, and the average number of rooms per dwelling. It appears that larger groups of lower status populations, homes that have less rooms, are less expensive, are closer to employment centers, and are older in age tend to have a trend with increased crime rates per capita. 
  
* **Question 10-d**: Do any of the census tracts of Boston appear to have particularly
high crime rates? Tax Rates? Pupil-teacher ratios? Comment on the range of each predictor. 
  * **Answer**: There are two suburbs that are greater than 3 standard deviations above the mean crime rate. There is a suburb that is greater than 3 standard deviations above the mean full-value property-tax rate per $10,000. There are no extreme outliers from the mean with respect to pupil-teacher ratios. The crime rates per capita range from 0.02% to 8.98%. The tax-rates range from 198 to 666. The pupil-teacher ratios range from 13.60 to 20.20.
...
```{r echo=FALSE}
plot_quantitative_summary_hist_freq_box_singular(boston_sm_tract_crime_rate, "crim", title = "Crime Rates Per Capita When Tract Bounds Charles River", x_axis_title = "Crime Rates Per Capita")
```

```{r echo=FALSE}
plot_quantitative_summary_hist_freq_box_singular(boston_sm_tract_crime_rate, "tax", title = "Full-Value Property-Tax Rate When Tract Bounds Charles River", x_axis_title = "Full-Value Property-Tax Rate Per $10,000")
```
```{r echo=FALSE}
plot_quantitative_summary_hist_freq_box_singular(boston_sm_tract_crime_rate, "ptratio", title = "Pupil-Teacher Ratio When Tract Bounds Charles River", x_axis_title = "Pupil-Teacher Ratio By")
```

* **Question 10-e**: How many of the census tracts in this data set bound the Charles river?
  * **Answer**: 
...
* **Question 10-f**: What is the median pupil-teacher ratio among the towns in this data set?
  * **Answer**: 
```{r}
print(sprintf("%0.0f is the median pupil-teacher ratio among the towns in this data set.", median(Boston$ptratio)))
```

* **Question 10-g**: Which census tract of Boston has lowest median value of owner-
...
The tract that has the lowest median value of owner-occupied homes has a per capita crime rate that is greater than the minimum per capita crime rate, but less than Q1 of the IQR of the per capita crime rate overall. 

This tract is in alignment with the most frequent (0) proportion of residential land zoned for lots over 25,000 sq. ft.

This tract is greater than the 3rd Quartile of the IQR of the overall proportion of non-retail business acres per town. This tract is less than the maximum of this overall proportion. 

This tract is the greatest nitrogen oxide concentration of the overall nitrogen oxide concentrations. 

This tract is midway between lower limit of 1.5 times the IQR and the 1st quartile with respect to the overall average number of rooms per dwelling. 

This tract is grouped with the largest proportion of owner-occupied units built before 1940.

This tract is grouped with the penultimate closest weighted mean distance to employment centers. 

This tract is within one standard deviation of the mean of the Full-Property Tax Rate Per $10,000. It is binned with at least 50 other suburbs. 

This tract is amongst the lowest pupil teacher ratios at 14.7. The overall values of pupil teacher ratios extend from a minimum of 12.6 to a maximum of 22.

This tract is among the larger percent of lower status of the population with a value of 26.82%. Overall values range from a minimum of 1.73% to a maximum of 37.97%. 

This tract is among the least expensive owner-occupied homes with a value of 13.4 thousand dollars. The overall minimum and maximum of the median value of owner-occupied homes in the $1000s are 5 and 50 respectively. 
  
The following plots show the suburb with the lowest median value of owner-occupied homes. 
  
```{r include = FALSE}
boston_sm_tracts_only <- Boston %>% filter(chas == 1)
boston_sm_tracts_only
lowest_median_tract <- boston_sm_tracts_only %>% filter(medv == min(boston_sm_tracts_only$medv))
```
  
```{r include = FALSE}
boston_formatted <- Boston %>% select(everything(), -chas, -rad)
lowest_median_tract_formatted <- lowest_median_tract %>% select(everything(), -chas, -rad)
```
...
titles <- c(
  "Lowest Median Tract: Per Capita Crime Rate By Town", 
  "Lowest Median Tract: Proportion of Residential Land", 
  "Lowest Median Tract: Proportion of Non-Retail Business Acres", 
  "Lowest Median Tract: Nitrogen Oxide Concentration", 
  "Lowest Median Tract: Average Number of Rooms Per Dwelling", 
  "Lowest Median Tract: Age of Homes", 
  "Lowest Median Tract: Mean Distance to Employment Centers",
  "Lowest Median Tract: Full-Value Property Tax Rate", 
  "Lowest Median Tract: Pupil-Teacher Ratio", 
  "Lowest Median Tract: Percent of Lower Status of Population", 
  "Lowest Median Tract: Median Value of Owner-Occupied Homes")
plot_quantitative_summary_hist_freq_box(boston_formatted, titles = titles, x_axis_titles = x_axis_titles, custom_scale = FALSE, sample_point = TRUE, sample_df = lowest_median_tract_formatted)
```

* **Question 10-h**: In this data set, how many of the census tracts average more than seven rooms per dwelling? More than eight rooms per dwelling? Comment on the census tracts that average more than eight rooms per dwelling. 
  * **Answer**: 
  
```{r echo = FALSE}
print(sprintf("In this data set, there are %0.0f census tracts that average more than 7 rooms per dwelling", nrow(Boston %>% filter(chas == 1 & rm > 7))))
print(sprintf("In this data set, there are %0.0f census tracts that average more than 8 rooms per dwelling", nrow(Boston %>% filter(chas == 1 & rm > 8))))
```

Both tracts are greater than the averages for the proportion of non-retail business acres per town, nitrogen oxygen concentration, average number of rooms per dwelling, the proportion of owner-occupied units built before 1940, the full-value property tax rate per $10,000, and the median value of owner-occupied homes in the $1000s. 

Similarly, both tracts are less than the averages for the per capita crime rate by town, the proportion of residential land zoned for lots over 25,000 sq.ft., the weighted mean distance to employment centers, and the percent of lower status of the population.

The tracts only differ with respect to the overall average of the pupil-teacher ratio by town where one tract is less than average and the other tract is greater. 

Plots of the individual tracts with respect to the overall data in the Boston data set are viewable below.   


```{r include = FALSE}
more_than_8_rooms <- Boston %>% filter(chas == 1 & rm > 8)
(more_than_8_rooms)
more_than_8_rooms_tract1_formatted <- more_than_8_rooms[1,] %>% select(everything(), -chas, -rad)
more_than_8_rooms_tract2_formatted <- more_than_8_rooms[2,] %>% select(everything(), -chas, -rad)
```
...
titles <- c(
  "1st Tract > 8 Rooms: Per Capita Crime Rate By Town", 
  "1st Tract > 8 Rooms: Proportion of Residential Land", 
  "1st Tract > 8 Rooms: Proportion of Non-Retail Business Acres", 
  "1st Tract > 8 Rooms: Nitrogen Oxide Concentration", 
  "1st Tract > 8 Rooms: Average Number of Rooms Per Dwelling", 
  "1st Tract > 8 Rooms: Age of Homes", 
  "1st Tract > 8 Rooms: Mean Distance to Employment Centers",
  "1st Tract > 8 Rooms: Full-Value Property Tax Rate", 
  "1st Tract > 8 Rooms: Pupil-Teacher Ratio", 
  "1st Tract > 8 Rooms: Percent of Lower Status of Population", 
  "1st Tract > 8 Rooms: Median Value of Owner-Occupied Homes")
plot_quantitative_summary_hist_freq_box(boston_formatted, titles = titles, x_axis_titles = x_axis_titles, custom_scale = FALSE, sample_point = TRUE, sample_df = more_than_8_rooms_tract1_formatted)
```
...
titles <- c(
  "2nd Tract > 8 Rooms: Per Capita Crime Rate By Town", 
  "2nd Tract > 8 Rooms: Proportion of Residential Land", 
  "2nd Tract > 8 Rooms: Proportion of Non-Retail Business Acres", 
  "2nd Tract > 8 Rooms: Nitrogen Oxide Concentration", 
  "2nd Tract > 8 Rooms: Average Number of Rooms Per Dwelling", 
  "2nd Tract > 8 Rooms: Age of Homes", 
  "2nd Tract > 8 Rooms: Mean Distance to Employment Centers",
  "2nd Tract > 8 Rooms: Full-Value Property Tax Rate", 
  "2nd Tract > 8 Rooms: Pupil-Teacher Ratio", 
  "2nd Tract > 8 Rooms: Percent of Lower Status of Population", 
  "2nd Tract > 8 Rooms: Median Value of Owner-Occupied Homes")
plot_quantitative_summary_hist_freq_box(boston_formatted, titles = titles, x_axis_titles = x_axis_titles, custom_scale = FALSE, sample_point = TRUE, sample_df = more_than_8_rooms_tract2_formatted)
```
