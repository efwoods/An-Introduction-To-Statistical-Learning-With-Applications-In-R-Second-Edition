...
evaluate_results <- function(model_list, test_list, selected_subject=NULL){
    if (is.null(selected_subject)){
        ...
        true_positive_rate_collection <- c(integer(length(model_list)))
        false_positive_rate_collection <- c(integer(length(model_list)))
        false_negative_rate_collection <- c(integer(length(model_list)))
    }
    for (i in seq(1, length(model_list))){
      if(is.null(selected_subject)){
        ...
        if(i == selected_subject){
          ...
          true_positive_rate_collection[[i]] <- (table[4]) / (table[4] + table[2]) * 100
          false_positive_rate_collection[[i]] <- table[2] / (table[4] + table[1]) * 100
          false_negative_rate_collection[[i]] <- table[1] / (table[1] + table[4]) * 100
    ...
    if (is.null(selected_subject)){
      ...
      mean_true_positive_rate <- mean(true_positive_rate_collection)
      f_print(sprintf("Mean TPR: %0.3f%%.", mean_true_positive_rate))
      cat("\n")
      
      TPR_standard_error <- sqrt(var(true_positive_rate_collection))
      f_print(sprintf("TPR Standard Error: ±%0.3f%%.", TPR_standard_error))
      cat("\n")

      mean_false_positive_rate <- mean(false_positive_rate_collection)
      f_print(sprintf("Mean FPR: %0.3f%%.", mean_false_positive_rate))
      cat("\n")
      
      FPR_standard_error <- sqrt(var(false_positive_rate_collection))
      f_print(sprintf("FPR Standard Error: ±%0.3f%%.", FPR_standard_error))
      cat("\n")

      mean_false_negative_rate <- mean(false_negative_rate_collection)
      f_print(sprintf("Mean FNR: %0.3f%%.", mean_false_negative_rate))
      cat("\n")
      
      FNR_standard_error <- sqrt(var(false_negative_rate_collection))
      f_print(sprintf("FNR Standard Error: ±%0.3f%%.", FNR_standard_error))
      cat("\n")
    }
...
# Step 2: Explore Subject 2's data and find outliers, high-leverage, or non-linearities.
```{r}
set.seed(42)
...
f_print(sprintf("Accuracy of Logistic Regression on Subject 2's Test Data after removing high-leverage: %0.3f%%.", glm.accuracy_test_no_high_leverage))
```
...
subject_2_svm_no_high_leverage.table <- table(prediction = subject_2_svm_no_high_leverage.pred, truth = mi_sit_R_AO_test_list[[2]]$y)
...
modellr %>% compile(loss = "categorical_cros", optimizer = optimizer_rmsprop(), metrics = c("accuracy"))
modellr %>% fit(mi_sit_R_AO_x_train_nn, mi_sit_R_AO_y_train_nn, epochs = 30, batch_size = , validation_split = 0.2)
```
...
```{r}
predict(modellr, mi_sit_R_AO_train_no_high_leverage[[2]][validation_set, ])
```
...
```{r}
data <- matrix(py$mi_sit_R_AO_x_train_formatted, byrow = TRUE)
# data
(data[[1,1]])
# py$mi_sit_R_AO_x_train_formatted$fb1
```
